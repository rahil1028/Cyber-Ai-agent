...
.grad{
    background:linear-gradient(90deg,#52b8ff,#8b5cff);
...
.shield{
    ...
    display:grid;
    place-items:center;
    background:radial-gradient(circle,#243c70,#0d1a2b 60%);
    font-size:110px
}
...
button{
    background:linear-gradient(90deg,#5a54ff,#874dff);
    color:white;
    border:0;
    border-radius:11px;
    padding:13px 18px;
...
<div class="nav">
    <div class="logo">🛡️ Cyber<span class="grad">Lens</span> AI</div>
...
<h1>
Your AI-powered
<span class="grad">Security Assistant</span>
</h1>

<p>
Analyze web-security scenarios, understand risk,
structure manual testing, and get practical remediation guidance.
</p>
...
window.analyze = async () => {
    ...
    try{
        ...
        if(response.ok){
            ...
            $("out").innerHTML =
                "<p>" + esc(data.error || "Request failed") + "</p>";
...
"""
    ...
    try:
        ...
        return jsonify(
            error="AI request failed; check server configuration."
        ), 500
