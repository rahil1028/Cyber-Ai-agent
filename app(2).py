...
async function boot(){
  try{
    ...
    if(!r.ok) throw new Error("Could not load Firebase configuration.");
    const c=await r.json();
    if(!c.apiKey||!c.authDomain||!c.projectId||!c.appId)
      throw new Error("Firebase configuration is incomplete.");
    const firebaseApp=initializeApp(c);
    ...
    $("msg").textContent="Firebase setup error. Please try again after configuration is fixed.";
  }
}
window.showAuth=()=>{$("landing").classList.add("hidden");$("auth").classList.remove("hidden")}
window.signup=async()=>{
  $("msg").textContent="";
  if(!A){$("msg").textContent="Firebase is still loading. Please wait a moment.";return}
  try{await createUserWithEmailAndPassword(A,$("email").value.trim(),$("pw").value)}
  catch(e){$("msg").textContent=e.message}
}
window.login=async()=>{
  $("msg").textContent="";
  if(!A){$("msg").textContent="Firebase is still loading. Please wait a moment.";return}
  try{await signInWithEmailAndPassword(A,$("email").value.trim(),$("pw").value)}
  catch(e){$("msg").textContent=e.message}
}
...
@app.get("/config")
def config():
    # Firebase Web SDK requires these exact property names.
    return jsonify({
