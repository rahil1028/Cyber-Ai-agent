...
<div class="wrap section" id="pricing">
    ...
    <div class="grid-3">
        <div class="feature-card">
            ...
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ Passive assessments</span></div>
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ AI security Q&amp;A with Gemini</span></div>
            <div class="step" style="margin-bottom:18px"><span class="muted">✓ Analysis &amp; scan history</span></div>
        ...
        <div class="feature-card" style="border-color:var(--accent);position:relative">
            <span class="status-badge" style="position:absolute;top:22px;right:22px">Coming soon</span>
            <h3>Pro</h3>
            <p style="margin:6px 0 18px">For teams that scan multiple sites regularly.</p>
            <div style="font-family:'Sora';font-size:32px;font-weight:800;margin-bottom:18px">Custom</div>
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ Active + scheduled assessments</span></div>
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ PDF report exports</span></div>
            <div class="step" style="margin-bottom:18px"><span class="muted">✓ Team accounts</span></div>
            <button class="btn-ghost btn-full" disabled style="opacity:.55;cursor:not-allowed">Notify me</button>
        </div>
        <div class="feature-card">
            <h3>Enterprise</h3>
            <p style="margin:6px 0 18px">For organizations with compliance needs.</p>
            <div style="font-family:'Sora';font-size:32px;font-weight:800;margin-bottom:18px">Contact us</div>
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ Dedicated onboarding</span></div>
            <div class="step" style="margin-bottom:10px"><span class="muted">✓ Priority support</span></div>
            <div class="step" style="margin-bottom:18px"><span class="muted">✓ Custom integrations</span></div>
            <button class="btn-ghost btn-full" onclick="showAuth()">Get started</button>
        </div>
    </div>
</div>
...
function renderScanFindings(findings, targetUrl){
    ...
    box.innerHTML = summaryHtml + findingsHtml +
        '<button type="button" class="btn-ghost btn-sm btn-full" style="margin-top:14px" onclick="downloadReport()">⬇ Download PDF report</button>';
...
window.downloadReport = function(){
    ...
    doc.setFontSize(18);
    doc.text("CyberLens AI — Security Report", 14, y);
    y += 10;

    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text("Target: " + target, 14, y); y += 6;
    doc.text("Generated: " + generatedAt.toLocaleString(), 14, y); y += 6;
    doc.text("High: " + counts.high + "   Medium: " + counts.med + "   Low: " + counts.low, 14, y); y += 10;
    ...
    findings.forEach((f, i) => {
        ...
        const title = (f.title || f.name || "Finding " + (i+1)) + "  [" + (f.severity || "info") + "]";
